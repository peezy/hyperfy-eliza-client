...
// src/client.ts
var hyperfyHandlerTemplate = `{{actionExamples}}
  ...
  constructor(port = 3001) {
    ...
    this.app.post(
      ...
      async (req, res) => {
        ...
        function createHyperfyOutSchema(body2) {
          const emotes = body2.emotes || [];
          const triggers = body2.triggers || [];
          const lookAtSchema = triggers.length > 0 ? z.union([
            z.union(
              triggers.map(
                (item) => z.literal(item)
              )
          ...
          const emoteSchema = emotes.length > 0 ? z.union([
            z.union(
              emotes.map(
                (item) => z.literal(item)
              )
...
var index_default = hyperfyPlugin;
export {
